...
def _generate_qiime_config(python_path, deploy_dir, all_apps_to_deploy, log,
    ...
    parallel_jobs_script = 'start_parallel_jobs.py'
    if qiime_path:
        # If we have a [qiime] target, write out additional QIIME config lines
        # for qiime_scripts_dir, etc.. These values have since been removed in
        # QIIME, but we'll continue to write them here to maintain backwards
        # compatibility with older qiime-deploy conf files that still deploy
        # QIIME instead of assuming that QIIME's python code is already
